...
class HeteroGraph:
    def __init__(self):
        ...
        self.train_labels = torch.zeros(self.get_user_num(), self.get_item_num())
        self.test_labels = torch.zeros(self.get_user_num(), self.get_item_num())
    ...
    def load_graph(self):
        ...
        self.G = dgl.heterograph(hetero_graph_dict)

    def add_node_features(self):
        self.G.nodes['item'].data['feat'] = torch.from_numpy(self.node_features[:3706, 1:])
        self.G.nodes['user'].data['feat'] = torch.from_numpy(self.node_features[:, 1:])
        self.G.nodes['kg'].data['feat'] = torch.cat((self.G.nodes['user'].data['feat'], torch.randn(self.kg_num, 16)))
        self.features = self.G.nodes['kg'].data['feat']

    def get_train_batch(self, batch_size):
        return self.train_graph.sample(batch_size).values
