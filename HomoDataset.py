...
class HomoGraph:
    def __init__(self):
        ...
        edges_group = self.train_graph.groupby(['label'])
        self.train_pos = edges_group.get_group(1)
        self.train_neg = edges_group.get_group(0)

    def load_graph(self):
        sources = self.origin_graph.iloc[:, 1].values
        sinks = self.origin_graph.iloc[:, 2].values
        graph = dgl.graph((sources, sinks))
        return graph
